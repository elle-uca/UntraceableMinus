...
public class ImportExportService {
    ...
    public List<Person> importFromCSV(JFrame parent, String dialogTitle) {
        ...
        File file = fileChooser.getSelectedFile();
        List<Person> importedPeople = new ArrayList<Person>();
    ...
    public void importWithDuplicatesCheck(final JFrame parent, final List<Person> importedPeople) {
        if (importedPeople == null || importedPeople.isEmpty()) {
            JOptionPane.showMessageDialog(parent, "No valid records found in the CSV.");
            return;
        }

        final JDialog progressDialog = createProgressDialog(parent, importedPeople.size());
        final JProgressBar progressBar = (JProgressBar) progressDialog.getContentPane().getComponent(1);
                    ...
                    if (!duplicates.isEmpty()) {
                        if (duplicateMode == -1) {
                            ...
                            for (Person p : duplicates) {
                                dupInfo += "- " + p.getName() + " " + p.getSurname();
                                if (p.getAddress() != null && !p.getAddress().isEmpty()) {
                                    dupInfo += " (" + p.getAddress() + ")";
                                }
                                dupInfo += "\n";
                            }
